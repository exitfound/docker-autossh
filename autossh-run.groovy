pipeline {
    ...
    parameters {
        ...
        string(name: 'SSH_TUNNEL_LOCALPORT', defaultValue: '80', trim: true, description: 'Локальный порт, представляющий собой сервис, который будет проброшен с конечного инстанса:')
        ...
        string(name: 'SSH_TUNNEL_REMOTEPORT', defaultValue: '9090', trim: true, description: 'Удаленный порт сервиса, который запущен на конечном инстансе и это тот сервис, который будет прокинут через Autossh:')
    }
